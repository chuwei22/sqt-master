...
/**
 * another soft drop function used by the ghost piece and dose not add points
 * @function
 * @memberof Tetris
 * @param {Tetris.Game} game The initial state of a game.
 * @returns {Tetris.Game} The state after a soft drop is attempted.
 */
Tetris.soft_drop_ghost = function (game) {
    if (Tetris.is_game_over(game)) {
        return game;
    }
    return descend(game);
...
const noOfLines = R.count(is_complete_line)
...
Tetris.next_turn = function (game) {
    ...
    //calculate and update score using the number of lines cleared
    Score.cleared_lines(noOfLines(locked_field), game.score);
